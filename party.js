// party.js — Login, Friends, DMs, Party Chat
...
(function () {
  ...
  var SESSION_KEY = 'mg67_session'; // {uid, name, emoji, passwordHash}
  var EMOJIS = ['🦊','🐉','🤖','🐺','🦁','🐯','🐸','🦋','🐬','🦄','🐻','🐙','🦅','🐼','🦉'];
  ...
  function fbSet(path, data, cb) {
    fetch(fbUrl(path), { method:'PUT', body:JSON.stringify(data), headers:{'Content-Type':'application/json'} })
      .then(function(){ if(cb) cb(); }).catch(function(){ if(cb) cb(); });
  }
  function fbPush(path, data, cb) {
    fetch(fbUrl(path), { method:'POST', body:JSON.stringify(data), headers:{'Content-Type':'application/json'} })
  ...
  function fbUpdate(path, data, cb) {
    fetch(fbUrl(path), { method:'PATCH', body:JSON.stringify(data), headers:{'Content-Type':'application/json'} })
      .then(function(){ if(cb) cb(); }).catch(function(){ if(cb) cb(); });
  }
  ...
  var currentDmId = null; // uid of DM partner

  // ══════════════════════════════════════════════
  // NAVBAR CLEANUP + BUTTON INJECT
  // ══════════════════════════════════════════════
  ...
  function injectButton() {
    ...
    badge.id = 'partyBadge';
    badge.style.cssText = 'display:none;position:absolute;top:-5px;right:-5px;background:#ff5c7a;color:#fff;font-size:9px;font-weight:800;min-width:16px;height:16px;border-radius:8px;align-items:center;justify-content:center;padding:0 3px;border:2px solid var(--bg,#080b12);pointer-events:none;';
    b.appendChild(badge);
    ...
    if (isPlay) {
      ...
      var panic = document.querySelector('.panic-btn,#panicBtn');
      ...
      fbGet('/dms', function(dms){
        if (dms) {
          Object.values(dms).forEach(function(conv){
            ...
            Object.values(conv.messages).forEach(function(msg){
              if (msg.to === me().uid && !msg.read) count++;
            });
          });
        }
  ...
  function togglePanel() {
    ...
    if (visible) hidePanel(); else openPanel();
  }
  ...
  function buildPanel() {
    if (document.getElementById('partyPanel')) {
      openPanel2(); return;
    }
    var panel = document.createElement('div');
    panel.id = 'partyPanel';
    panel.style.cssText = [
      'position:fixed;top:78px;right:16px;width:375px;max-height:600px;',
      'background:var(--bg3,#111827);border:1px solid var(--border2,rgba(255,255,255,0.11));',
      'border-radius:16px;box-shadow:0 24px 64px rgba(0,0,0,0.85);z-index:9999;',
    ...
    panel.innerHTML =
      '<div id="pHeader" style="padding:12px 14px 10px;border-bottom:1px solid var(--border,rgba(255,255,255,0.06));flex-shrink:0;">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">' +
          '<span id="pTitle" style="font-size:15px;font-weight:800;font-family:Syne,sans-serif;color:var(--text,#f0f4ff);">🎉 Party &amp; Friends</span>' +
          '<div style="display:flex;gap:6px;align-items:center;">' +
            '<span id="pUserChip" style="font-size:11px;color:var(--text2);"></span>' +
  ...
  function buildTabs(active) {
    ...
    tabs.innerHTML = list.map(function(t){
      var on = t.v === active;
      return '<button class="ptab" data-view="'+t.v+'" style="flex:1;padding:5px 3px;border-radius:8px;border:1px solid '+(on?'rgba(108,143,255,0.35)':'var(--border2,rgba(255,255,255,0.11))')+';background:'+(on?'rgba(108,143,255,0.15)':'none')+';color:'+(on?'var(--accent3,#a5bcff)':'var(--text2)')+';font-size:10px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;">'+t.l+'</button>';
    }).join('');
    tabs.querySelectorAll('.ptab').forEach(function(t){
    ...
    body.innerHTML =
      '<div style="padding:20px;">' +
        '<div style="text-align:center;margin-bottom:20px;">' +
          ...
          '<div style="font-size:16px;font-weight:800;color:var(--text,#f0f4ff);font-family:Syne,sans-serif;">Welcome back!</div>' +
          '<div style="font-size:12px;color:var(--text2);margin-top:4px;">Log in to chat with friends</div>' +
        '</div>' +
        field('loginUser', 'text', 'Username', '') +
        field('loginPass', 'password', 'Password', '') +
        '<button id="loginBtn" style="'+bigBtn('var(--accent,#6c8fff)')+'">Log In</button>' +
        '<div id="loginErr" style="font-size:12px;color:#ff7c94;text-align:center;margin-top:8px;min-height:16px;"></div>' +
        '<div style="text-align:center;margin-top:14px;font-size:12px;color:var(--text2);">No account? <button id="goRegister" style="background:none;border:none;color:var(--accent3,#a5bcff);cursor:pointer;font-size:12px;font-weight:700;font-family:inherit;">Create one</button></div>' +
      '</div>';
    ...
    body.innerHTML =
      '<div style="padding:20px;">' +
        '<div style="text-align:center;margin-bottom:16px;">' +
          ...
          '<div style="font-size:16px;font-weight:800;color:var(--text,#f0f4ff);font-family:Syne,sans-serif;">Create Account</div>' +
        '</div>' +
        field('regUser', 'text', 'Username (max 20 chars)', '') +
        field('regPass', 'password', 'Password', '') +
        field('regPass2', 'password', 'Confirm password', '') +
        '<label style="font-size:12px;color:var(--text2);display:block;margin-bottom:8px;font-weight:700;">Pick your emoji</label>' +
        '<div id="emojiPicker" style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:14px;">' +
          EMOJIS.map(function(e,i){
            return '<button class="regEmoji" data-e="'+e+'" style="font-size:18px;padding:4px;background:'+(i===0?'rgba(108,143,255,0.2)':'rgba(255,255,255,0.04)')+';border:1px solid '+(i===0?'rgba(108,143,255,0.4)':'rgba(255,255,255,0.08)')+';border-radius:7px;cursor:pointer;line-height:1;">'+e+'</button>';
          }).join('') +
        '</div>' +
        '<button id="regBtn" style="'+bigBtn('var(--accent,#6c8fff)')+'">Create Account</button>' +
        '<div id="regErr" style="font-size:12px;color:#ff7c94;text-align:center;margin-top:8px;min-height:16px;"></div>' +
        '<div style="text-align:center;margin-top:14px;font-size:12px;color:var(--text2);">Already have one? <button id="goLogin" style="background:none;border:none;color:var(--accent3,#a5bcff);cursor:pointer;font-size:12px;font-weight:700;font-family:inherit;">Log in</button></div>' +
      '</div>';
  ...
  function doRegister(getEmoji) {
    ...
    fbGet('/users', function(users){
      ...
      fbSet('/users/'+uid, {
        uid: uid, name: uname, emoji: emoji,
  ...
  function renderLobby(body) {
    body.innerHTML =
      '<div style="padding:10px 14px;border-bottom:1px solid var(--border,rgba(255,255,255,0.06));flex-shrink:0;">' +
        '<button id="showCreateBtn" style="'+bigBtn('var(--accent,#6c8fff)')+'">+ Create a Party</button>' +
      '</div>' +
  ...
  function loadLobby() {
    fbGet('/parties', function(data){
      ...
      list.innerHTML = parties.map(function(p){
        ...
        return '<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid var(--border,rgba(255,255,255,0.06));">'+img+'<div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:700;color:var(--text,#f0f4ff);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+esc(p.name)+'</div><div style="font-size:11px;color:var(--text2);margin-top:1px;">by '+esc(p.host)+' · '+members.length+' member'+(members.length!==1?'s':'')+'</div></div>'+ab+'</div>';
      }).join('');
  ...
  function showCreateForm(body) {
    ...
    body.innerHTML =
      '<div style="padding:16px;">' +
        ...
        field('cfInput','text','Party name...',game?game.title+' party':'') +
        '<div style="display:flex;gap:8px;">' +
          ...
          '<button id="cfSubmit" style="'+bigBtn('var(--accent,#6c8fff)')+'flex:2;">🎉 Create</button>' +
        '</div>' +
        '<div id="cfErr" style="font-size:12px;color:#ff7c94;margin-top:8px;min-height:14px;"></div>' +
      '</div>';
  ...
  function doJoin(partyId) {
    ...
    fbGet('/parties/'+partyId+'/members', function(members){
      ...
      fbSet('/parties/'+partyId+'/members/'+gid(), {
        uid:session.uid, name:session.name, emoji:session.emoji, joinedAt:Date.now()
      }, function(){
        fbPush('/parties/'+partyId+'/messages', {
          author:'System', emoji:'👋', text:session.name+' joined!', ts:Date.now(), system:true
        }, function(){ openPartyChat(partyId); });
      });
    });
  }
    ...
    body.innerHTML =
      '<div style="flex-shrink:0;background:rgba(108,143,255,0.04);border-bottom:1px solid var(--border,rgba(255,255,255,0.06));">' +
        '<div style="display:flex;align-items:center;gap:8px;padding:10px 14px;">' +
          ...
          '<div style="flex:1;min-width:0;"><div id="chatTitle" style="font-size:13px;font-weight:700;color:var(--text,#f0f4ff);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Loading...</div><div id="chatSubs" style="font-size:11px;color:var(--text2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"></div></div>' +
          '<div id="chatAct"></div>' +
        '</div>' +
      '</div>' +
      ...
      fbGet('/parties/'+partyId, function(p){
        ...
        if(aEl && !actionBuilt) {
          ...
          if(p.hostUid===session.uid) {
            aEl.innerHTML='<button id="disbandBtn" style="font-size:10px;padding:3px 10px;border-radius:6px;background:rgba(255,92,122,0.1);border:1px solid rgba(255,92,122,0.25);color:#ff7c94;cursor:pointer;font-family:inherit;">Disband</button>';
            document.getElementById('disbandBtn').onclick=function(){
              if(!confirm('Disband party?')) return;
              fbUpdate('/parties/'+partyId,{active:false},function(){ stopPoll(); showView('lobby'); });
            };
          } else {
            aEl.innerHTML='<button id="leaveBtn" style="font-size:10px;padding:3px 10px;border-radius:6px;background:rgba(255,255,255,0.06);border:1px solid var(--border2);color:var(--text2);cursor:pointer;font-family:inherit;">Leave</button>';
            document.getElementById('leaveBtn').onclick=function(){
              fbGet('/parties/'+partyId+'/members',function(mems){
                ...
                fbPush('/parties/'+partyId+'/messages',{author:'System',emoji:'👋',text:session.name+' left.',ts:Date.now(),system:true},function(){
                  stopPoll(); showView('lobby');
                });
              });
            };
          }
      ...
      fbGet('/users/'+session.uid+'/friends', function(friendData){
        ...
        if(pending.length){
          ...
          pending.forEach(function(entry){
            ...
            html+='<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid var(--border,rgba(255,255,255,0.06));background:rgba(108,143,255,0.04);">'+
              '<span style="font-size:24px;flex-shrink:0;">'+esc(r.fromEmoji||'👤')+'</span>'+
              '<div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:700;color:var(--text,#f0f4ff);">'+esc(r.fromName)+'</div><div style="font-size:11px;color:var(--text2);">wants to be friends</div></div>'+
        ...
        if(friendIds.length){
          ...
          friendIds.forEach(function(fid){
            ...
            html+='<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid var(--border,rgba(255,255,255,0.06));">'+
              '<span style="font-size:26px;flex-shrink:0;">'+esc(f.emoji||'👤')+'</span>'+
              '<div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:700;color:var(--text,#f0f4ff);">'+esc(f.name)+'</div></div>'+
              '<button data-fid="'+fid+'" data-fn="'+esc(f.name)+'" class="dmBtn" style="font-size:11px;font-weight:700;padding:4px 10px;border-radius:7px;background:rgba(108,143,255,0.12);border:1px solid rgba(108,143,255,0.3);color:var(--accent3,#a5bcff);cursor:pointer;font-family:inherit;margin-right:4px;">💬 DM</button>'+
  ...
  function doAddFriend() {
    ...
    var session=me();
    if(targetName===session.name){ setMsg(msgEl,"You can't add yourself!",'#ff7c94'); return; }
    setMsg(msgEl,'Searching...','var(--text2)');
    fbGet('/users',function(users){
      if(!users){ setMsg(msgEl,'User not found.','#ff7c94'); return; }
      ...
      if(targetId===session.uid){ setMsg(msgEl,"You can't add yourself!",'#ff7c94'); return; }
      fbGet('/users/'+session.uid+'/friends/'+targetId,function(ex){
        if(ex){ setMsg(msgEl,"You're already friends!",'#ff7c94'); return; }
        fbGet('/friendRequests',function(reqs){
          var sent=reqs&&Object.values(reqs).some(function(r){ return r.fromId===session.uid&&r.toId===targetId&&r.status==='pending'; });
          if(sent){ setMsg(msgEl,'Request already sent!','#ff7c94'); return; }
          fbPush('/friendRequests',{
            fromId:session.uid,fromName:session.name,fromEmoji:session.emoji,
            toId:targetId,toName:found[1].name,status:'pending',ts:Date.now()
          },function(){
            setMsg(msgEl,'✓ Request sent to '+targetName+'!','#00e5a0');
  ...
  function doRemove(fid){
    ...
    var session=me();
    fbDelete('/users/'+session.uid+'/friends/'+fid);
    fbDelete('/users/'+fid+'/friends/'+session.uid);
  }
  ...
  function renderDMs(body) {
    ...
    function loadDMs() {
      fbGet('/users/'+session.uid+'/friends', function(friends){
        ...
        var html = '<div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--text3);padding:10px 14px 4px;">💬 Direct Messages</div>';
        Object.entries(friends).forEach(function(entry){
          var fid=entry[0], f=entry[1];
          html += '<div data-fid="'+fid+'" data-fn="'+esc(f.name)+'" class="dmRow" style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid var(--border,rgba(255,255,255,0.06));cursor:pointer;transition:background 0.12s;">'+
            '<span style="font-size:26px;flex-shrink:0;">'+esc(f.emoji||'👤')+'</span>'+
            '<div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:700;color:var(--text,#f0f4ff);">'+esc(f.name)+'</div><div style="font-size:11px;color:var(--text2);">Tap to chat</div></div>'+
            '<span style="font-size:18px;color:var(--accent3,#a5bcff);">›</span>'+
          '</div>';
        });
        list.innerHTML = html;
        list.querySelectorAll('.dmRow').forEach(function(row){
          row.onmouseenter = function(){ row.style.background='rgba(108,143,255,0.06)'; };
  ...
  function renderDM(body, friendUid, friendName) {
    var session = me();
    var convId = dmConvId(session.uid, friendUid);

    // Get friend info if we only have uid
    function buildDMUI(fName, fEmoji) {
      body.innerHTML =
        '<div style="flex-shrink:0;background:rgba(108,143,255,0.04);border-bottom:1px solid var(--border,rgba(255,255,255,0.06));">' +
          '<div style="display:flex;align-items:center;gap:8px;padding:10px 14px;">' +
            '<button id="dmBack" style="background:none;border:none;color:var(--text2);cursor:pointer;font-size:13px;padding:0;font-family:inherit;flex-shrink:0;">← Back</button>' +
            '<span style="font-size:22px;">'+esc(fEmoji||'👤')+'</span>' +
            '<div style="font-size:14px;font-weight:700;color:var(--text,#f0f4ff);">'+esc(fName)+'</div>' +
          '</div>' +
        '</div>' +
      ...
      function sendDM() {
        ...
        var text=inp.value.trim(); inp.value='';
        fbPush('/dms/'+convId+'/messages',{
          from:session.uid, fromName:session.name, fromEmoji:session.emoji,
          to:friendUid, text:text, ts:Date.now(), read:false
        });
      }
      ...
      function pollDM(){
        fbGet('/dms/'+convId+'/messages', function(data){
          ...
          if(!data){ el.innerHTML='<div style="text-align:center;color:var(--text3);font-size:13px;padding:28px 0;margin:auto;">No messages yet — say hi! 👋</div>'; return; }
          var msgs=Object.values(data).sort(function(a,b){return a.ts-b.ts;});
          if(msgs.length===lastCount) return;
          lastCount=msgs.length;
          // Mark unread as read
          Object.entries(data).forEach(function(e){
            if(e[1].to===session.uid&&!e[1].read) fbUpdate('/dms/'+convId+'/messages/'+e[0],{read:true});
          });
          var atBottom=el.scrollHeight-el.scrollTop<=el.clientHeight+60;
          el.innerHTML=msgs.map(function(m){
            ...
            return '<div style="margin-bottom:10px;display:flex;flex-direction:column;align-items:'+(isMe?'flex-end':'flex-start')+';">'+
              '<div style="font-size:11px;color:var(--text3);margin-bottom:3px;">'+(isMe?'You':esc(m.fromName))+' · '+ago(m.ts)+'</div>'+
              '<div style="font-size:13px;padding:8px 12px;border-radius:12px;max-width:80%;word-break:break-word;background:'+(isMe?'linear-gradient(135deg,var(--accent,#6c8fff),#4a6ee8)':'rgba(255,255,255,0.08)')+';color:'+(isMe?'#fff':'var(--text,#f0f4ff)')+';">'+esc(m.text)+'</div>'+
            '</div>';
          }).join('');
          if(atBottom) el.scrollTop=el.scrollHeight;
        });
    ...
    fbGet('/users/'+friendUid, function(u){
      buildDMUI(u?u.name:friendName, u?u.emoji:'👤');
    });
  }
  ...
  function renderProfile(body) {
    ...
    body.innerHTML=
      '<div style="padding:16px;">'+
        '<div style="text-align:center;padding:16px;background:rgba(108,143,255,0.05);border-radius:12px;margin-bottom:16px;">'+
          ...
          '<div style="font-size:16px;font-weight:800;color:var(--text,#f0f4ff);">'+esc(session.name)+'</div>'+
          '<div style="font-size:11px;color:var(--text2);margin-top:4px;background:rgba(108,143,255,0.1);display:inline-block;padding:2px 10px;border-radius:20px;">@'+esc(session.name)+'</div>'+
        '</div>'+
        '<label style="font-size:12px;color:var(--text2);display:block;margin-bottom:8px;font-weight:700;">Change emoji</label>'+
        '<div id="profileEmojis" style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:16px;">'+
          EMOJIS.map(function(e){
            return '<button class="profEmoji" data-e="'+e+'" style="font-size:18px;padding:4px;background:'+(e===session.emoji?'rgba(108,143,255,0.2)':'rgba(255,255,255,0.04)')+';border:1px solid '+(e===session.emoji?'rgba(108,143,255,0.4)':'rgba(255,255,255,0.08)')+';border-radius:7px;cursor:pointer;line-height:1;">'+e+'</button>';
          }).join('')+
        '</div>'+
        '<button id="saveEmojiBtn" style="'+bigBtn('var(--accent,#6c8fff)')+'margin-bottom:10px;">Save Changes</button>'+
        '<div id="profMsg" style="font-size:12px;text-align:center;min-height:14px;"></div>'+
        '<hr style="border:none;border-top:1px solid var(--border,rgba(255,255,255,0.06));margin:16px 0;" />'+
        '<button id="logoutBtn" style="width:100%;background:rgba(255,92,122,0.1);border:1px solid rgba(255,92,122,0.25);color:#ff7c94;padding:10px;border-radius:9px;cursor:pointer;font-size:13px;font-weight:700;font-family:inherit;">Log Out</button>'+
      '</div>';
    ...
    body.querySelectorAll('.profEmoji').forEach(function(b){
      b.onclick=function(){
        ...
        b.style.background='rgba(108,143,255,0.2)'; b.style.borderColor='rgba(108,143,255,0.4)'; selEmoji=b.dataset.e;
      };
    });

    document.getElementById('saveEmojiBtn').onclick=function(){
      var s=me(); s.emoji=selEmoji; saveSession(s);
      fbUpdate('/users/'+s.uid,{emoji:selEmoji});
      setMsg(document.getElementById('profMsg'),'✓ Saved!','#00e5a0');
    };
    document.getElementById('logoutBtn').onclick=function(){
      if(!confirm('Log out?')) return;
      clearSession(); stopPoll(); showView('login');
    };
  }
  ...
  function renderMessages(msgs, myName) {
    if(!msgs.length) return '<div style="text-align:center;color:var(--text3);font-size:13px;padding:28px 0;margin:auto;">No messages yet — say hi! 👋</div>';
    return msgs.map(function(m){
      ...
      return '<div style="margin-bottom:8px;">'+
        '<div style="display:flex;align-items:baseline;gap:5px;margin-bottom:2px;">'+
          ...
          '<span style="font-size:12px;font-weight:700;color:'+(m.author===myName?'var(--accent3,#a5bcff)':'var(--text,#f0f4ff)')+';">'+esc(m.author)+'</span>'+
  ...
  function inputStyle() {
    return 'width:100%;background:rgba(108,143,255,0.06);border:1px solid var(--border2,rgba(255,255,255,0.11));border-radius:9px;padding:10px 14px;color:var(--text,#f0f4ff);font-size:13px;outline:none;font-family:inherit;box-sizing:border-box;';
  }
  function bigBtn(color) {
    return 'display:block;width:100%;background:linear-gradient(135deg,'+color+',#4a6ee8);border:none;color:#fff;padding:10px;border-radius:9px;cursor:pointer;font-size:13px;font-weight:700;font-family:inherit;box-shadow:0 4px 14px rgba(108,143,255,0.3);';
  }
